...
class solver():
    ...
    def __init__(self):
        ...
        self._discarded = set()
        self.allowed = self.words.allowed
        self.answers = self.words.answers
    ...
    def _filter_words(self) -> None:
        ...
        self.answers = [word for word in self.answers if self._is_valid(word)]
    ...
    def choose_word(self) -> str:
        ...
        words_x_letter = self.words.answers_by_letter(self.answers)
        best_exploratory_word = max(self.allowed, key=lambda x: self._how_many_discards(words_x_letter, x))
        best_valid_word = max(self.answers, key=lambda x: self._how_many_discards(words_x_letter, x))
